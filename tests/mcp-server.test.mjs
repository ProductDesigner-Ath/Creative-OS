...
test('MCP advertises unique controlled composition and asset tools',async()=>{const s=session();try{const init=await s.send({jsonrpc:'2.0',id:1,method:'initialize',params:{}});assert.equal(init.result.serverInfo.name,'creative-os-local-mcp');const list=await s.send({jsonrpc:'2.0',id:2,method:'tools/list'});const names=list.result.tools.map(tool=>tool.name);for(const name of ['ae_open_composition','ae_import_local_asset','ae_add_project_item_layer','ae_move_layer_before','ae_set_parent','ae_add_text_position_reveal','ae_animate_mask_feather','ae_add_gaussian_blur']) assert.ok(names.includes(name));assert.equal(new Set(names).size,names.length);}finally{s.p.kill();}});
